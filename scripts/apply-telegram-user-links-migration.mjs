/**
 * Применяет supabase/create_telegram_user_links.sql (нужен SUPABASE_ACCESS_TOKEN).
 */
...
if (!token) {
  console.error(
    [
      ...
      "Затем: npm run db:apply-telegram-user-links",
    ].join("\n"),
  );
  process.exit(1);
}
...
const sqlPath = path.join(root, "supabase", "create_telegram_user_links.sql");
...
console.log("telegram_user_links / user_can_access_board_for_user применены.");
if (text && text !== "{}") {
