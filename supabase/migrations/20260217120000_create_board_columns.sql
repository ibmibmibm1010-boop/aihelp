...
create index if not exists board_columns_board_sort_idx
  on public.board_columns (board_id, sort_order);
...
as $$
begin
  insert into public.board_columns (board_id, title, color, linked_status, sort_order)
  values
    (new.id, 'К выполнению', '#6366f1', 'todo', 0),
    (new.id, 'В работе', '#f59e0b', 'doing', 1),
    (new.id, 'Готово', '#22c55e', 'done', 2);
  return new;
end;
$$;
...
cross join lateral (
  values
    ('К выполнению', '#6366f1', 'todo', 0),
    ('В работе', '#f59e0b', 'doing', 1),
    ('Готово', '#22c55e', 'done', 2)
) as v(title, color, st, ord)
where not exists (
